#!/usr/bin/perl
# $Id: liberate.pl,v 1.1 2012/05/02 18:15:21 charliep Exp $
...
if ($ARGV[1]){
	...
	croak "No drive specified for liberation. Exiting...";
}

my $Bccd = new Bccd();

	if($Bccd->get_stage() eq 'LIVE') {

		open(my $LIB, '-|', '/usr/bin/sudo',
				'/usr/bin/perl', '/root/liberate.pl',
				'--libdev=$drive') or
			croak "Can't open /root/liberate.pl: $!\n";
